...
> **⚠️ TRAMPA DE EXAMEN:** El default de `ON_ERROR` **cambia**: COPY masivo = `ABORT_STATEMENT`; Snowpipe = `SKIP_FILE`. Y no confundas `CONTINUE` (omite **filas** malas) con `SKIP_FILE` (omite el **archivo** entero).
