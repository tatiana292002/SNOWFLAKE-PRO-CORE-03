...
> **💡 PIÉNSALO ASÍ:** El **user stage** es tu cajón personal; el **table stage** es el cajón pegado a un mueble (esa tabla); el **named stage** es una bodega compartida a la que puedes dar llaves a quien quieras.

## Cargar y descargar
- Subes con **`PUT`** y bajas con **`GET`** (desde SnowSQL o Snowflake CLI).
- Snowflake **cifra** los archivos en reposo automáticamente.

> **⚠️ TRAMPA DE EXAMEN:** Solo el **named stage** es un objeto "de verdad" al que puedes **otorgar privilegios** y asociar un **file format**. Los stages de usuario y de tabla **no** se granulan ni se comparten; si el caso pide control de acceso o compartir el stage, la respuesta es **named internal stage**.
