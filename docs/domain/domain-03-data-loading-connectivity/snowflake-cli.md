...
La **CLI moderna de Snowflake** (comando `snow`): ejecuta SQL, **gestiona objetos**, sube/baja archivos y **despliega proyectos** (Snowpark, Native Apps, Streamlit) desde la terminal. El **temario [...])
...
> **⚠️ TRAMPA DE EXAMEN:** Si preguntan por la **CLI moderna recomendada** para desplegar un proyecto Snowpark en un pipeline → **Snowflake CLI**. Si el caso solo menciona `PUT`/`GET` desde [...])
